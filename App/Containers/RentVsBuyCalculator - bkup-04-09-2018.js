...
export default class RentVsBuyCalculator extends Component{
	...
	callBuyerProrationSettingApi(){
		...
		.then((response) => {
			...
			console.log("prorationLength callBuyerProrationSettingApi rent v/s buy " + prorationLength);
		...
		callGetApi(GLOBAL.BASE_URL + GLOBAL.national_global_setting, {
			...
			.then((response) => {		
				...
				if(this.state.tab == 'FHA'){
					...
					data        = {'salePrice': newText,'LTV': ltv, 'MIP': mip, 'fhaMaxLoanAmount' : this.state.fhaMaxLoanAmount};
	...
	changeMonTaxPrice(){
		...
		console.log("responseRealEstate changeMonTaxPrice rent v/s buy " + JSON.stringify(responseRealEstate));


		//console.log(this.response.prepaidMonthTaxes);
		realEstateTaxesRes        = responseRealEstate.realEstateTaxes;

		console.log("realEstateTaxesRes changeMonTaxPrice rent v/s buy " + JSON.stringify(realEstateTaxesRes));
    ...
    calTotalMonthlyPayment(){
       ...
       requestTotPreItem         = {'principalRate': this.state.principalRate, 'realEstateTaxesRes': this.state.realEstateTaxesRes, 'homeOwnerInsuranceRes': this.state.homeOwnerInsuranceRes, 'monthlyRate': this.state.monthlyRate, 'pnintrate': this.state.pnintrate, 'paymentAmount1': this.state.paymentAmount1, 'paymentAmount2': this.state.paymentAmount2};
		...
		console.log("requestTotPreItem rent v/s buy " + JSON.stringify(requestTotPreItem));
		...
		this.setState({totalMonthlyPayment: responseTotPreItem.totalMonthlyPayment, totalMonthlyPaymentOld: responseTotPreItem.totalMonthlyPayment},this.changePrepaidPageFields);
    ...
    // this function is called when you change value from dropdown in case of Lender.
	 ...
	 //######################  End Seller Closing Costs ########################## 
		 ...
		 calFinalFunction() {
			...
			console.log("ten_years_sp_array " + JSON.stringify(this.state.ten_years_sp_array));
			...
			//		(parseFloat(this.state.totalSellerClosingCost)).toFixed(2); // Sale Cost at year X (use CF ACTUAL COSTS, not 8% used here!)
					...
					console.log("ten_years_sp_array 1  " + this.state.ten_years_sp_array[i + 1]);
							...
							<View style={RentVsBuyStyle.subHeaderNewDesign}>
									...
									<View style={{flexDirection: 'row', zIndex : -1}}>
										<View style={{paddingRight:10,paddingLeft:10,paddingTop:5,width:'100%',paddingTop:5}}>
											<CustomTextInput customKeyboardType="hello" name="sale_pr" ref="sale_pr"  underlineColorAndroid = 'transparent' style={RentVsBuyStyle.headerTextInputField} selectTextOnFocus={ true } onKeyPress={() => this.onFocus('sale_pr')} onChangeText={(value) => this.setState({sale_pr: this.onChange(value)})} onBlur={() => this.onBlur("homePrice")}
