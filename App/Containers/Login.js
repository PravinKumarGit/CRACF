...
import { View, Text, StyleSheet, Image, TextInput, Alert, ScrollView, TouchableOpacity, Dimensions, NetInfo, AsyncStorage, BackHandler} from 'react-native';
...
callValidation(){
	if(this.state.username == '') {
	 ...
	 if(this.state.password == "") {
		 this.setState({passwordError : STRINGS.t('blank_password_acc_error_message')});
...
render() {
	...
	if(this.state.showPage == "true") {
			showable=<View style={Styles.backgroundImage}>
			<View>
			   ...
			   <Image style={Styles.logoBackGround1} source={Images.costFirstText} />
				...
				<ScrollView style={Styles.scroll_container}>
				 <View style={Styles.viewContainer}>
				  ...
				  placeholder={this.state.usernameError == "" ? STRINGS.t('Username') : this.state.usernameError} underlineColorAndroid = 'transparent' secureTextEntry={false} style={Styles.textInput}  returnKeyType ="next" keyboardType="email-address"  onChangeText={(val) => this.setState({username: val,usernameError:''})} />
				  { this.state.usernameError != '' ? <Icon name='ios-alert' onPress={() => this.onIconClick(this.state.usernameError)} style={{color:'red'}}/> : null }
				  </View>
				  <View style={Styles.lineViewTextBox}></View>
				  { this.state.usernameError != '' ? <View style={Styles.lineErrorView}></View> : <View style={Styles.lineView}></View> }
				  <View style={Styles.backgroundViewContainer}>
						...
						placeholder={this.state.passwordError == "" ? STRINGS.t('Password') : this.state.passwordError} underlineColorAndroid = 'transparent' secureTextEntry={true} style={Styles.textInput}  onChangeText={(val) => this.setState({password: val,passwordError:''})}/>		  
						{ this.state.passwordError != '' ? <Icon onPress={() => this.onIconClick(this.state.passwordError)} name='ios-alert' style={{color:'red'}}/> : null }
				  </View>
				  <View style={Styles.lineViewTextBox}></View>
				  { this.state.passwordError != '' ? <View style={Styles.lineErrorView}></View> : <View style={Styles.lineView}></View> }
				 ...
				 <TouchableOpacity style={Styles.buttonContainer} onPress={this.onLoginPress.bind(this)}>
				  ...
				  <TouchableOpacity onPress={this.moveToForgotUsername.bind(this)}>
					
				 <Text style={Styles.usernameAndPasswordStyles}> {STRINGS.t('USERNAME')}</Text>
				  </TouchableOpacity>
				  <Text style={Styles.slashStyle}> / </Text>
				  <TouchableOpacity onPress={this.moveToForgotPassword.bind(this)}>
